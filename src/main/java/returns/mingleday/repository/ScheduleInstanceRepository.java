...
public interface ScheduleInstanceRepository extends JpaRepository<ScheduleInstance, Long> {
    ...
    @Query("select si from ScheduleInstance si where si.schedule.mingle in :mingles and si.startAt < :end and si.endAt >= :start and (si.schedule.isPrivate = false or si.schedule.owner = :user) order by si.startAt asc, si.endAt asc, si.schedule.title asc")
    List<ScheduleInstance> findAllByOneDay(User user, List<Mingle> mingles, LocalDateTime start, LocalDateTime end);
}
