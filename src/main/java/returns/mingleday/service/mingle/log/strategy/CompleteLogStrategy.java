...
@Component
public class CompleteLogStrategy implements CreateMingleLogInterface {
    @Override
    public MingleLog create(Mingle mingle, MingleMember operator, Object target) {
        ...
        String content = operator.getDisplayName() + "님이 " + schedule.getTitle() + " 일정을 완료 처리했습니다.";
        ...
        return MingleLog.ofTarget(
                ...
                schedule.getTitle(),
                content,
                MingleLogType.COMPLETE
        );
    }

    @Override
    public MingleLogType getMingleLogType() {
        return MingleLogType.COMPLETE;
    }
}
